...
int main()
{
	...
	if (ETIMEDOUT == pthread_cond_wait(&deinitCond, &deinitMutex)) //ceka
	{
...
void remoteControllerCallback(uint16_t code, uint16_t type, uint32_t value)
{
    switch(code)
	{
		case KEYCODE_INFO:
            printf("\nInfo pressed\n");          
            if (getChannelInfo(&channelInfo) == SC_NO_ERROR) //daje broj kanla
            {
                printf("\n********************* Channel info *********************\n");
			...
			break;
		case KEYCODE_P_PLUS:   //typdef-ovi za bojev kanala
			printf("\nCH+ pressed\n");
            channelUp();
			break;
